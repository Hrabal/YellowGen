...
def weight_list(list_from, most_probable=None, smoothness = None):
    l = len(list_from)
    y = most_probable or list_from[l/2]
    z = smoothness or 1
    weights = [x +  10 * (1/l-abs(abs(y-x))) * z for x in range(1,l)]
...
durations = zip(durations,weight_list(durations,3))
print durations
#Weighted list of octaves
octaves = zip(range(0,9),weight_list(range(0,9),3))
print zip(range(0,9),weight_list(range(0,9),3,5))
print zip(range(0,9),weight_list(range(0,9),3))
...
'''
    def __repr__(self):
        return ''.join([''.join(x)[:40]+'\n' for x in self.score])
'''
class Song(object):
    song_parts = [('verse',(2,4)),('chorus',(2,4)),('prechorus',(1,3)),('solo',(1,2)),('intro',(1,1)),('outro',(1,1))]
    """docstring for Song"""
